...
#seems to be the stacked method as suggested in F.4 
    ...
    def log_prob_tensor(self, x: Wordtype, y: Wordtype, z: Wordtype) -> TorchScalar:
        ...
        log_prob = logit_for_trigram - torch.log(Z) #normalize we need to consider log of the partition function
        # TODO: IMPLEMENT ME!
    ...
    def logits(self, x: Wordtype, y: Wordtype, z: Wordtype) -> torch.Tensor:
        ...
        #
        x_emb = self.embeddings(x, self.embeddings.get("OOL"))
        y_emb = self.embeddings(y, self.embeddings.get("OOL"))
        z_emb = self.embeddings(z, self.embeddings.get("OOL"))
    ...
    def train(self, file: Path, C: float, E: int):    # type: ignore
        ...
        for e in range(E):
            ...
            for i, trigram in enumerate(tqdm(read_trigrams(file), total=N)): # To get the training examples, you can use the `read_trigrams` function
                gamma = gamma0/(1+gamma0*2*C*t/N) #current step size
                
                #compute the forward 
                log_prob = self.log_prob_tensor(trigram)  #need tensor for bookkeeping instead of float 
                Fi_theta = log_prob - C*(self.X.norm()+self.Y.norm())#For each successive training example i, compute the stochastic
